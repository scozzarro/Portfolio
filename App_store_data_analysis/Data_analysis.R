...
#1.0 Load Data----
path_main<- "App_store_data_analysis/Data/AppleStore.csv"
path_desc<- "App_store_data_analysis/Data/appleStore_description.csv"
...
levels(unified_raw_tbl$prime_genre)
...
unified_raw_tbl %>% group_by(prime_genre) %>%
                    ...
                    mutate(prime_genre_2= case_when(pct < 0.01 ~ "Other", TRUE ~ prime_genre)) %>%
                    group_by(prime_genre_2) %>%
                    summarise(pct = sum(pct), mrkt_val = sum(mrkt_val)) %>%
                    ggplot(aes(mrkt_val, 
                               reorder(prime_genre_2, mrkt_val), 
                               fill = prime_genre_2,
